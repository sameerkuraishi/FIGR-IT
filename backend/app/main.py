...
def recommend(skills: Dict[str, int], goal: str = ""):
    ...
    for career, weights in CAREERS.items():
        contributions = {
            skill: round(skills.get(skill, 0) * weight)
        ...
        results.append(
            {
                "career": career,
                "score": min(99, sum(contributions.values())),
    ...
    rows = [
        {
            ...
            "required": required,
            "deficit": max(0, required - skills.get(skill, 0)),
        }
...
@app.get("/api/health")
def health():
    return {"status": "ready", "mode": "deterministic-demo"}
...
def save_profile(profile: Profile):
    return {"stored": True, "profile": profile}
...
def assessment(data: Assessment):
    score = round(
        sum(
            answer == data.correct_answers[index]
...
def analyze(profile: Profile):
    return {
        ...
        "recommendations": recommend(profile.skills, profile.goal),
        "priority_gaps": gaps(profile.skills)[:3],
    }


@app.post("/api/career/recommend")
def career_recommend(profile: Profile):
    return {"recommendations": recommend(profile.skills, profile.goal)}
...
def roadmap_generate(profile: Profile):
    ...
    return {
        "target": profile.goal,
        "nodes": [
            {"topic": topic, "reason": "priority skill deficit"} for topic in weak
        ]
        + [{"topic": "Guided project", "reason": "evidence-building"}],
    }
...
def tutor_chat(data: TutorRequest):
    ...
    return {
        "answer": text,
        "retrieved_from": ["statistics.json", "machine_learning.json"],
        "fallback_used": True,
...
def quiz_submit(result: QuizResult):
    ...
    return {
        "topic": result.topic,
        "score": percent,
        "status": "weak" if percent < 60 else "mastered",
    }
...
def roadmap_adapt(result: QuizResult):
    ...
    if percent >= 60:
        return {
            "updated": False,
            "score": percent,
            "message": "Continue with the current roadmap.",
        }
    return {
        "updated": True,
        "score": percent,
        "detected_gap": "Probability fundamentals",
        "inserted_nodes": [
            "Statistics revision",
