...
export function AlumniConnectStep({
  ...
  const [tab, setTab] = useState<
    "discover" | "connections" | "mentorship"
  >("discover");
  ...
  const [message, setMessage] = useState(
    ...
    } from FIGR IT. I'd love to learn about your journey and get some career guidance.`
  );
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
              Discover verified alumni, build professional
              connections and ask for practical mentorship
              on careers, projects, placements and higher
              studies.
            </p>
          </div>
          <div className="grid grid-cols-3 gap-2 sm:gap-3">
            <div className="rounded-xl bg-[var(--secondary)] px-4 py-3 text-center">
              <strong className="block text-xl text-[var(--winter-dark)]">
                {alumniData.length}
              </strong>
          ...
          <section className="panel p-4">
            <div className="flex flex-col gap-3 md:flex-row">
              ...
              <select
                ...
                className="h-10 rounded-md border border-[var(--input)] bg-white px-3 text-sm"
              >
                {years.map((item) => (
          ...
          {visible.length === 0 ? (
            <section className="panel p-10 text-center">
              ...
              <p className="mt-1 text-sm text-[var(--muted-foreground)]">
                Try another company, role, skill or graduation year.
              </p>
            </section>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-1.5">
                            ...
                            {alumni.verified && (
                              <ShieldCheck className="h-4 w-4 text-[var(--winter-success)]" />
                            )}
                          </div>
                      ...
                      <span className="rounded-full bg-[var(--secondary)] px-2.5 py-1 text-[10px] font-bold">
                    ...
                    <div className="mt-3">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {alumni.availableFor.map((item) => (
                          <span
                            key={item}
                            className="rounded-full bg-[var(--secondary)] px-2.5 py-1 text-[11px]"
                          >
                            {item}
                          </span>
                        ))}
                    ...
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {alumni.skills.map((skill) => (
                        <span
                          key={skill}
                          className="rounded-full border border-[var(--border)] px-2.5 py-1 text-[11px]"
                        >
                          {skill}
                        </span>
                      ))}
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          <div className="panel p-5 lg:col-span-2">
            ...
            <h2 className="mt-1 text-2xl font-bold">
              Get practical guidance, not generic advice.
            </h2>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-3">
              {[
                [
                  "01",
                  "Find a match",
                  "Filter alumni by role, company, branch and skills.",
                ],
                [
                  "02",
                  "Connect",
                ...
                [
                  "03",
                  "Learn",
                  "Ask focused questions or request a short mentorship session.",
                ],
              ].map(([number, title, text]) => (
                <div
                  key={number}
                  className="rounded-xl bg-[var(--winter-bg)] p-4"
                >
                  <span className="text-xs font-black text-[var(--winter-primary)]">
                    {number}
                  </span>
          ...
          <div className="panel p-5">
            ...
            <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
              Ask one specific question at a time. This helps
              alumni give more useful and actionable guidance.
            </p>
          </div>
        </section>
      )}
