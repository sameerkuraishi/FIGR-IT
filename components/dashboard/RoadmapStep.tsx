...
import { Route, LockKeyhole, Check, Trophy, BookOpen, ChevronRight, Download } from "lucide-react";
...
export function RoadmapStep({
  selectedCareerId, setStep, activeCareer, weakest, profile,
  completedModules, roadmap, completeModule
}: RoadmapStepProps) {
  ...
  if (!selectedCareerId) {
    return (
      <EmptyState icon={Route} title="No roadmap yet"
        ...
        action="Go to Careers" onAction={() => setStep("careers")} />
    );
  }
  ...
  const downloadPDF = () => {
    const pdf = new jsPDF();
    pdf.setFontSize(20);
    pdf.text("FIGR IT - Career Roadmap", 20, 20);
    pdf.setFontSize(12);
    pdf.text(`Student: ${profile.name || "Student"}`, 20, 32);
    pdf.text(`Career: ${activeCareer.title}`, 20, 40);
    pdf.text(`Study time: ${profile.weeklyHours} hours/week`, 20, 48);
    pdf.text(`Budget: ${profile.budget}`, 20, 56);
    ...
    roadmap.forEach((module, index) => {
      ...
      pdf.setFontSize(14);
      pdf.text(`${index + 1}. ${module.title}`, 20, y);
      pdf.setFontSize(10);
      pdf.text(`Type: ${module.type}`, 25, y + 7);
      pdf.text(`Timeline: ${module.weeks} | Duration: ${module.duration}`, 25, y + 14);

      const lines = pdf.splitTextToSize(`Objective: ${module.objective}`, 165);
      pdf.text(lines, 25, y + 21);
      pdf.text(`Resource: ${module.resource}`, 25, y + 21 + lines.length * 5);
      y += 34 + lines.length * 5;
    });

    pdf.save("FIGR-IT-Career-Roadmap.pdf");
  };
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-[10px] uppercase font-bold text-[var(--winter-primary)] tracking-widest mb-2">
              Step 7 · Try the real work
            </p>
            <h1 className="text-3xl font-heading font-semibold text-[var(--winter-dark)]">
              {activeCareer.title} Roadmap
            </h1>
            <p className="text-[var(--muted-foreground)] mt-2 max-w-2xl">
              A 12-week visual journey shaped by the {weakest.skill.toLowerCase()} gap,
              {profile.weeklyHours} study hours per week and {profile.budget.toLowerCase()}.
            </p>
          </div>
                ...
                <article key={module.title} className={`relative flex gap-6 ${done ? "opacity-60" : ""} ${locked ? "opacity-40 grayscale" : "hover:-translate-y-1 transition-transform"}`}>
                  <button onClick={() => completeModule(index)}
                    ...
                    className={`absolute -left-[45px] w-10 h-10 rounded-full flex items-center justify-center shrink-0 shadow-sm border-2 transition-colors ${
                      locked ? "bg-[var(--secondary)] border-[var(--border)] text-[var(--muted-foreground)]"
                      : done ? "bg-[var(--winter-success)] border-[var(--winter-success)] text-white"
                      : index === roadmap.length - 1 ? "bg-amber-100 border-amber-400 text-amber-600"
                  ...
                  <div className={`flex-1 p-5 rounded-2xl border ${done ? "border-[var(--border)] bg-[var(--secondary)]" : locked ? "border-[var(--border)] bg-white/50" : "border-[var(--winter-primary)]/30 bg-white shadow-md ring-1 ring-[var(--winter-primary)]/10"}`}>
                    <div className="flex flex-wrap items-center gap-3 mb-2">
                      ...
                      <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-[var(--secondary)] text-[var(--muted-foreground)] border border-[var(--border)]">{module.type}</span>
                    ...
                    <footer className="flex items-center justify-between pt-4 border-t border-[var(--border)] text-xs text-[var(--winter-dark)]">
                      <div className="flex items-center gap-2"><BookOpen className="w-4 h-4 text-[var(--winter-primary)]"/><span className="font-semibold">{module.resource}</span></div>
        ...
        <aside className="flex flex-col gap-6">
          ...
          <section className="premium-card p-6 md:p-8 bg-gradient-to-tr from-[var(--winter-dark)] to-slate-800 text-white">
            ...
            <div className="flex flex-col gap-3">
              ...
              <Button variant="outline" onClick={() => setStep("adaptation")} className="border-white/20 text-white hover:bg-white/10 h-12">
                Go to Adaptation <ChevronRight className="w-4 h-4 ml-2"/>
              </Button>
            </div>
          </section>
        </aside>
