  ...
  return careers
    .map((item) => {
      ...
      const requirementFit =
        (coreSkills.reduce((sum, skill) => sum + Math.min(1, skills[skill] / item.requirements[skill]), 0) /
          coreSkills.length) *
        100;
      ...
      const interestAlignment = Math.min(100, Math.round(tagMatches * 22 + trackMatch * 35));
      const match = Math.round(skillEvidence * 0.45 + requirementFit * 0.35 + interestAlignment * 0.2);
